...
<body>
    ...
    $name = $_POST['search'];
    $link = connect();
    $rel = 'SELECT * FROM pages WHERE user_id ="' . $_SESSION['id'] . '" AND page_name ="' . $name . '"  ORDER BY crdate';
    $res = mysqli_query($link, $rel); #тут все норм  
    $another = 'SELECT COUNT(*) FROM pages WHERE user_id ="' . $_SESSION['id'] . '" AND page_name ="' . $name . '" ';
